...
export const LeagueAccordion: React.FC<LeagueAccordionProps> = ({ league, isExpanded, onToggle }) => {
  ...
  return (
    <div className={`bg-white rounded-xl border transition-all ${isExpanded ? 'border-accent shadow-lg' : 'border-slate-200 shadow-sm hover:border-accent'}`}>
      <button 
        onClick={onToggle}
        className="w-full text-left p-6 flex justify-between items-center group gap-4"
      >
        <div className="flex items-start gap-4 flex-1">
          <div className={`p-2 rounded-lg mt-1 transition-colors flex-shrink-0 ${isExpanded ? 'bg-accent text-white' : 'bg-slate-50 text-slate-400 group-hover:bg-accent/10 group-hover:text-accent'}`}>
            <Users size={20} />
          </div>
          <div className="min-w-0 flex-1">
            <h3 className={`text-lg font-bold transition-colors truncate ${isExpanded ? 'text-accent' : 'text-primary'}`}>{league.name}</h3>
            {league.description && (
            ...
            <div className="flex items-center gap-2 text-[10px] text-slate-400 font-bold uppercase tracking-wider mt-2">
              <span className="flex items-center gap-1"><Users size={10} /> {league.membersCount} medlemmar</span>
              <span>•</span>
              <div 
                onClick={handleCopy}
        ...
        <div className="flex-shrink-0">
          {isExpanded ? <ChevronDown size={24} className="text-accent" /> : <ChevronRight size={24} className="text-slate-300 group-hover:text-accent" />}
        </div>
      </button>
